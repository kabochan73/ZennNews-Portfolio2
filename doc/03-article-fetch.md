...
| 実行方法 | Railway Cron が `php artisan zenn:fetch-articles` を実行する。コマンドは現在の日本時間を見て、該当する `fetch_hour` のタグを取得する（3〜8時以外は何もしない）。タグの間は1秒あける |
| 手動実行 | `--tag=nextjs`（指定したタグ）、`--hour=5`（指定した時間の担当タグ）、`--all`（全タグ）で実行できる |
| 終了コード | 1つでも失敗したタグがあれば失敗（1）を返す。Railway の実行履歴で失敗として見えるようにするため |
...
| キャッシュの作り直しの通知 | タグを1つ取り終えるたびに、Next.js の `POST /api/revalidate` にタグの slug を送り、そのタグのホーム（`/home/[slug]`、ISR）を作り直させる。Next.js と共有する秘密の値をヘッダーに付けて送る |
| 通知の失敗 | ログに残すだけで、取得は続ける。Next.js 側でも8時間ごとに作り直すので、最大8時間で新しい記事が反映される |
